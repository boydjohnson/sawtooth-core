...
impl BlockManager {
    ...
    fn transaction_index_contains(
        &self,
        index: &Box<IndexedBlockStore>,
        block_id: &str,
        id: &str,
    ) -> Result<bool, BlockManagerError> {
        let index = index.as_ref();
        if TransactionIndex::contains(index, id)? {
            let block = TransactionIndex::get_block_by_id(index, id)?
    ...
    fn batch_index_contains(
        ...
        let index = index.as_ref();
        if BatchIndex::contains(index, id)? {
            let block = BatchIndex::get_block_by_id(index, id)?
